...
class RGBCompWidth(val rWidth: Int, val gWidth: Int, val bWidth: Int)

class RGB(compWidth: RGBCompWidth, width: Int, alignedWidth: Int) extends Bundle {
...
class RGBFactory(val rWidth: Int, val gWidth: Int, val bWidth: Int) {
  ...
  def apply() = new RGB(new RGBCompWidth(rWidth, gWidth, bWidth), width, alignedWidth)

  def decode(pix: UInt) = {
    val res = Wire(new RGB(new RGBCompWidth(rWidth, gWidth, bWidth), width, alignedWidth))
